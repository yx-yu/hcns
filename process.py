...
def skip_code_and_nl_with_skip_id(data_dir, output_dir, is_skipped):
    # skip data.
    nls = []
    with open(data_dir + 'nl.original', 'r', encoding='utf-8') as f:
        for line_index, line in enumerate(f.readlines()):
            if not is_skipped[line_index]:
                nls.append(line)

    codes = []
    with open(data_dir + 'code.seq', 'r', encoding='utf-8') as f:
        for line_index, line in enumerate(f.readlines()):
            if not is_skipped[line_index]:
                codes.append(line)
    ...
    with open(output_dir + 'nl.original', 'w', encoding='utf-8') as f:
    ...
    with open(output_dir + 'code.seq', 'w', encoding='utf-8') as f:
